# ======================================================================
# SG6 SCREENER — APP STREAMLIT V18
...
def calcular_gex(ticker_name, precio_actual):
    try:
        ...
        if precio_actual > call_wall: emoji, ctx = "🔴","Techo gamma"
        elif precio_actual < put_wall: emoji, ctx = "🟡","Piso gamma"
        else:
            d1 = precio_actual - put_wall
            d2 = call_wall - precio_actual
            emoji, ctx = ("🟠","Cerca techo") if d2 < d1*0.4 else ("🟢","Zona favorable")
        return {"Put Wall": f"${put_wall:.2f}", "Call Wall": f"${call_wall:.2f}", "GEX": f"{emoji} {ctx}"}
    except Exception:
        return None
...
def analizar_ticker(ticker_name, periodo):
    ...
    df['S_MACD_CROSS'] = (df['CROSS_MACD']&(df['DIF']<0)&(df['HISTO']>df['HISTO'].shift(1))&(df['Close']>df['MA20'])&df['MA20_UP']&(df['Volume']>df['VOL_MA']*1.05)&df['FILTRO_BASE'])
    ...
    df['S_BOLL_SOFT'] = ((df['Low'].shift(1)<=df['BB_DN'])&(df['Close']>df['BB_DN'])&(df['Close']>df['Low'].shift(1))&df['GIRO_J']&(df['Volume']>df['VOL_MA']*0.9)&df['FILTRO_BASE']&~df['MACD_GIRO_NEG'])
    ...
    # B_SIGNAL: primer dia de cada racha de B_RAW=True
    df['B_SIGNAL'] = df['B_RAW'] & ~df['B_RAW'].shift(1).fillna(False)

    # Dias consecutivos activa contados desde atras
    ...
    # ================================================================
    # PRECIO DE ENTRADA CORREGIDO
    # Busca el B_SIGNAL que inició la racha activa actual
    # Es el primer True de B_RAW en la racha que llega hasta hoy
    # ================================================================
    e_price, atr_entry, duracion = None, None, 0
    if dias_activa > 0:
        # El inicio de la racha actual está hace (dias_activa-1) barras
        idx_entrada = len(df) - dias_activa
        if idx_entrada >= 0:
            e_price   = float(df['Close'].iloc[idx_entrada])
            atr_entry = float(df['ATR_V'].iloc[idx_entrada])
            duracion  = dias_activa - 1  # dias transcurridos desde entrada
    ...
    i = len(df) - 1
    add_bc = (dias_activa>0 and banda=="BC" and df['PDI_A'].iloc[i]>df['MDI_A'].iloc[i] and
              df['ADX_A'].iloc[i]>28 and bool(df['ADX_A_ACCEL'].iloc[i]) and
              bool(df['CROSS_KD'].iloc[i]) and df['KVAL'].iloc[i]<75 and
              df['Close'].iloc[i]>df['MA20'].iloc[i] and df['Volume'].iloc[i]>df['VOL_MA'].iloc[i]*1.2)
    add_vol = (dias_activa>0 and banda=="VOL" and df['PDI_A'].iloc[i]>df['MDI_A'].iloc[i] and
               df['ADX_A'].iloc[i]>32 and bool(df['ADX_A_ACCEL'].iloc[i]) and
               df['Close'].iloc[i]>df['High'].iloc[max(0,i-3):i].max() and
               df['Close'].iloc[i]>df['MA20'].iloc[i] and df['Volume'].iloc[i]>df['VOL_MA'].iloc[i]*1.4)
    ...
    if dias_activa > 0:
        for nombre, col in [
            ...
            ("S_REBOTE_MA50","S_REBOTE_MA50"),("S_REBOTE_MA200","S_REBOTE_MA200"),("S_BOLL_SOFT","S_BOLL_SOFT"),
        ]:
...
with st.sidebar:
    ...
    st.markdown('<p style="color:#334155;font-size:11px;text-align:center;margin-top:8px;">SG6 · v18.0 · Solo fines educativos</p>', unsafe_allow_html=True)
...
for idx, ticker in enumerate(tickers):
    ...
    if es_compra or es_add:
        ...
        lista_compras.append({
            ...
            "Call Wall": call_wall,
            "GEX":       gex_ctx,
            "_falta":    datos["pct_falta_raw"],
            "_sl":       datos["pct_sl_raw"],
        })
    ...
    if datos["Radar_BTD"]:
        lista_radar.append({"Ticker":datos["Ticker"],"Señal":"🟢 BTD","Banda":datos["Banda"],"Precio":datos["Precio"],"RSI":datos["RSI"],"J(V)":datos["J(V)"],"BB_DN":datos["BB_DN"],"Put Wall":put_wall,"Call Wall":call_wall,"GEX":gex_ctx})
    elif datos["Radar_OJO"]:
        lista_radar.append({"Ticker":datos["Ticker"],"Señal":"🟡 OJO","Banda":datos["Banda"],"Precio":datos["Precio"],"RSI":datos["RSI"],"J(V)":datos["J(V)"],"BB_DN":datos["BB_DN"],"Put Wall":put_wall,"Call Wall":call_wall,"GEX":gex_ctx})
...
# Métricas — una sola vez, debajo del banner
col1, col2, col3 = st.columns(3)
col1.metric("🚀 Compras + ADD", len(lista_compras))
col2.metric(f"📉 RSI < {rsi_umbral}", len(lista_rsi))
col3.metric("🎯 Radar OJO/BTD", len(lista_radar))
...
GLOSARIO_COMPRAS = """<div class="glosario">
<b>Banda</b> — BC (baja volatilidad) · VOL (alta volatilidad) &nbsp;|&nbsp;
<b>Antigüedad</b> — días desde la vela gatillo &nbsp;|&nbsp;
<b>Gatillos</b> — señales que activaron la entrada · ⚡ = ADD sobre posición abierta &nbsp;|&nbsp;
<b>P.Entrada</b> — precio de cierre de la vela gatillo (fijo, no cambia) &nbsp;|&nbsp;
<b>Precio</b> — precio actual de mercado &nbsp;|&nbsp;
<b>RSI</b> — momentum: rojo &lt;33 · verde &gt;65 &nbsp;|&nbsp;
<b>ADX</b> — fuerza de tendencia (&gt;25 relevante) &nbsp;|&nbsp;
<b>TP1</b> — objetivo calculado desde P.Entrada + ATR × multiplicador &nbsp;|&nbsp;
<b>Falta TP1</b> — distancia del precio actual al objetivo · ✅ = ya superado &nbsp;|&nbsp;
<b>SL −3%</b> — stop loss fijo: 3% bajo P.Entrada &nbsp;|&nbsp;
<b>Dist. SL</b> — margen actual al stop · rojo = peligro &nbsp;|&nbsp;
<b>Put/Call Wall</b> — soporte/resistencia gamma opciones 5 semanas &nbsp;|&nbsp;
<b>GEX</b> — 🟢 zona favorable · 🟠 cerca techo · 🔴 techo activo · 🟡 piso gamma
</div>"""

GLOSARIO_RSI = """<div class="glosario">
<b>RSI</b> — Wilder: rojo &lt;33 sobreventa extrema &nbsp;|&nbsp;
<b>J(V)</b> — componente J del KDJ: &lt;20 zona de posible giro alcista &nbsp;|&nbsp;
<b>ADX</b> — fuerza de la tendencia dominante &nbsp;|&nbsp;
<b>Put/Call Wall</b> — soporte y resistencia de opciones 5 semanas &nbsp;|&nbsp;
<b>GEX</b> — contexto gamma actual
</div>"""

GLOSARIO_RADAR = """<div class="glosario">
<b>OJO 🟡</b> — alerta temprana: RSI &lt;35 + J(V) &lt;28 + precio cerca banda inferior Bollinger. Zona de vigilancia, no señal de compra &nbsp;|&nbsp;
<b>BTD 🟢</b> — Buy The Dip confirmado: ayer fue OJO + hoy J cruza al alza + MACD gira. Señal más madura &nbsp;|&nbsp;
<b>BB_DN</b> — banda inferior Bollinger: soporte técnico dinámico
</div>"""

# SECCIONES COMO EXPANDERS
st.markdown("---")

with st.expander(f"🚀 Compras + ADD  ({len(lista_compras)})", expanded=True):
    ...
    if lista_compras:
        ...
        df1 = df1.sort_values('_ord').drop(columns=['_ord','_falta','_sl']).reset_index(drop=True)
...
with st.expander(f"📉 Sobreventa RSI  ({len(lista_rsi)})", expanded=False):
    st.markdown(GLOSARIO_RSI, unsafe_allow_html=True)
    if lista_rsi:
        df2 = pd.DataFrame(lista_rsi).sort_values("RSI").reset_index(drop=True)
        df2.index = range(1, len(df2)+1)
        styled2 = df2.style.map(colorear_banda,subset=["Banda"]).map(colorear_rsi,subset=["RSI"])
...
with st.expander(f"🎯 Radar OJO / BTD  ({len(lista_radar)})", expanded=False):
    ...
    if lista_radar:
        ...
        styled3 = df3.style.map(colorear_banda,subset=["Banda"]).map(colorear_rsi,subset=["RSI"])

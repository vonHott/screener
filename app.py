# ======================================================================
# SG3 SCREENER — APP STREAMLIT V9
# Archivo: app.py
# ======================================================================
# NOVEDADES V9:
#   - Nuevo gatillo S_REBOTE_MA50 (volumen relajado, solo BC)
#   - Log de descartados integrado como sección colapsable
#   - Descripcion de gatillos en pantalla de inicio
...
with st.sidebar:
    ...
    st.markdown("---")
    st.caption("SG3 v9 · RSI Wilder · BC/VOL Adaptativo")
...
st.caption("Sistema KW-DNA · BC/VOL Adaptativo · RSI Wilder · 7 Gatillos · v9.0")
...
| S_REBOTE_MA50 (nuevo v9) | Rebote limpio en MA50, solo BC | 95% |
        """)
    st.stop()
    ...
    try:
        ...
        if 0 < datos["Dias_Activa"] <= dias_max:
            ...
            lista_compras.append({
                "Ticker": datos["Ticker"], "Fecha": datos["Fecha"],
                "Banda": datos["Banda"], "Antiguedad": estado_str,
        ...
        if datos["RSI"] < rsi_umbral:
            lista_rsi.append({
                "Ticker": datos["Ticker"], "Fecha": datos["Fecha"],
                "Banda": datos["Banda"], "Precio": datos["Precio"],
                "RSI": datos["RSI"], "J_V": datos["J_V"], "ADX": datos["ADX"],
            })

        if datos["Radar_BTD"]:
            lista_radar.append({
                "Ticker": datos["Ticker"], "Senal": "BTD",
                "Banda": datos["Banda"], "Precio": datos["Precio"],
                "RSI": datos["RSI"], "J_V": datos["J_V"], "BB_DN": datos["BB_DN"],
            })
        elif datos["Radar_OJO"]:
            lista_radar.append({
                "Ticker": datos["Ticker"], "Senal": "OJO",
                "Banda": datos["Banda"], "Precio": datos["Precio"],
                "RSI": datos["RSI"], "J_V": datos["J_V"], "BB_DN": datos["BB_DN"],
            })
...
with tab1:
    ...
    if lista_compras:
        ...
        st.download_button("Descargar CSV", df1.to_csv(index=False).encode("utf-8"),
...
with tab2:
    ...
    if lista_rsi:
        ...
        st.download_button("Descargar CSV", df2.to_csv(index=False).encode("utf-8"),
...
with tab3:
    ...
    if lista_radar:
        ...
        st.download_button("Descargar CSV", df3.to_csv(index=False).encode("utf-8"),
                           "radar_sg3_v9.csv", "text/csv")
    else:
        st.info("Sin alertas tempranas de formacion de suelo.")

if lista_desc:
    with st.expander(f"Tickers descartados ({len(lista_desc)})", expanded=False):
        df4 = pd.DataFrame(lista_desc).reset_index(drop=True)
        st.dataframe(df4, use_container_width=True, hide_index=True)

st.markdown("---")
st.caption("SG3 Screener v9 · RSI Wilder sincronizado con Moomoo/TradingView · Solo con fines educativos.")
